...
export default function Player() {
  ...
  // Fungsi untuk mengirim semua episode SATU PER SATU - FIXED: menggunakan state untuk tracking progress
  const sendAllToTelegram = useCallback(async (bots) => {
    ...
    try {
      // Process episodes sequentially tanpa function dalam loop
      for (let i = 0; i < episodesWithUrl.length; i++) {
        ...
        // Update progress modal dengan episode saat ini
        ...
        // Kirim ke semua bot yang dipilih
        for (const bot of bots) {
          try {
            // Gunakan token dari environment variable atau fallback
            const botToken = bot.token.replace('***SENSORED***', 
        ...
        // Update success/failed count menggunakan functional update
        setProgressModal(prev => ({
          ...prev,
          successCount: prev.successCount + (episodeSuccess ? 1 : 0),
          failedCount: prev.failedCount + (episodeSuccess ? 0 : 1)
        }));

        // Delay tambahan antar episode
        await new Promise(resolve => setTimeout(resolve, 500));
      }
      
      // Ambil final counts dari state untuk alert
      const finalSuccess = progressModal.successCount;
      const finalFailed = progressModal.failedCount;
      
      // Tampilkan hasil akhir
      if (!isCancelledRef.current) {
        alert(`📊 Pengiriman selesai!\n✅ Berhasil: ${finalSuccess}\n❌ Gagal: ${finalFailed}`);
      } else {
  ...
  }, [dramaData.episodes, dramaData.info, isAuthenticated, openPasswordModal, progressModal.successCount, progressModal.failedCount]);
